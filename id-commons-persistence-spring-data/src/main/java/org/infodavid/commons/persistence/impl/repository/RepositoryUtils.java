...
public class RepositoryUtils {
    ...
    public <T> Predicate toPredicate(final CriteriaBuilder criteriaBuilder, final Root<T> root, final Restriction restriction) {
        ...
        case NOT_EQ: {
            predicate = criteriaBuilder.equal(root.get(restriction.getField()), restriction.getValue()).not();

            break;
        }
        case NOT_LIKE: {
            predicate = criteriaBuilder.like(root.get(restriction.getField()), toString(restriction.getValue())).not();
        ...
        case LIKE: {
            predicate = criteriaBuilder.like(root.get(restriction.getField()), toString(restriction.getValue()));
