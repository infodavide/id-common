...
public class UserRepositoryTest extends AbstractSpringTest {
    ...
    public void testFindAllUsingPagination() throws Exception {
        ...
        for (final User result : results.getResults()) {
            ...
            assertNotNull("Null groups", result.getGroups());
            ...
            if ("anonymous".equals(result.getName())) {
                ...
                assertFalse("Empty groups", result.getGroups().isEmpty());
                assertNotNull("No group", result.getGroups().get("admins"));
                assertEquals("Wrong number of properties for user: " + result.getName(), 2, result.getProperties().size());
            } else {
                assertFalse("Empty groups", result.getGroups().isEmpty());
                assertNotNull("No group", result.getGroups().get("users"));
    ...
    public void testFindAllUsingPaginationAndRestriction() throws Exception {
        ...
        final Page<User> results = dao.findAll(params, Restriction.with("role", RestrictionOperator.LIKE, "US%"));

        assertNotNull("Null results", results);
        assertNotNull("Null results", results.getResults());
        assertEquals("Wrong results", 1, results.getNumber());
        assertEquals("Wrong results", 3, results.getTotalSize());
    ...
    public void testInsert() throws Exception {
        ...
        assertNotNull("Null groups", inserted.getGroups());
        assertEquals("Wrong groups", result.getGroups().keySet(), inserted.getGroups().keySet());
